...
2. Comment out the Aiven Cloud `db.url`, `db.user`, and `` lines (add a `#` at the beginning of those lines).
3. Uncomment the "Backup Localhost Connection" lines (remove the `#`).
4. If your local MySQL `root` user has a password, update the `=` field to match your password. *(Example: `=root123`)*
